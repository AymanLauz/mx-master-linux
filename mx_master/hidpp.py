...
class HIDPPDevice:
    ...
    def _send_recv(self, report: list[int], timeout_ms: int = 1000) -> bytes:
        ...
        deadline = time.monotonic() + timeout_ms / 1000
        while time.monotonic() < deadline:
            resp = self._read(timeout_ms=100)
            if resp is None:
                continue
    ...
    def get_feature_index(self, feature_code: int) -> int:
        ...
        report = [SHORT, DEVICE_INDEX, 0x00, (0 << 4) | SW_ID, high, low, 0x00]
    ...
    def _reprog_get_count(self, feat_idx: int) -> int:
        report = [SHORT, DEVICE_INDEX, feat_idx, (0 << 4) | SW_ID, 0x00, 0x00, 0x00]
        resp = self._send_recv(report)
        return resp[4]
    ...
    def _reprog_divert_control(self, feat_idx: int, ctrl_id: int) -> None:
        ...
        hi = (ctrl_id >> 8) & 0xFF
        lo = ctrl_id & 0xFF
        # setControlReport (function 3): ctrl_id, reporting_ctrl_id, flags (0x01 = divert)
        report = [LONG, DEVICE_INDEX, feat_idx, (3 << 4) | SW_ID,
                  hi, lo, hi, lo, 0x01, 0x00,
                  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]
    ...
    def unlock_buttons(self) -> None:
        ...
        for i in range(count):
            ...
            if self._debug:
                print(f"  control[{i}]: ctrl_id={ctrl_id:#06x} task_id={info['task_id']:#06x}")
            self.controls[ctrl_id] = info

        # Divert controls we want to handle ourselves.
        # These task_ids are standard across Logitech devices:
        #   0x00C4 = SmartShift toggle (mode shift button)
        #   0x00D7 = Gesture button
        #   0x00DC = Thumb wheel click
        DIVERT_TASK_IDS = {0x00C4, 0x00D7, 0x00DC}
        for ctrl_id, info in self.controls.items():
            if info["task_id"] in DIVERT_TASK_IDS:
                if self._debug:
